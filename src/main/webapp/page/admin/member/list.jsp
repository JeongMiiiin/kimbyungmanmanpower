						...
						<table class="col-12">
							...
							<tbody>
							    ...
							    <tr>
							    	...
							    	<td>${list.phone0}${list.phone1}${list.phone2}</td>
							    	<td>
							    		<a href="/adm/member/update?sn=${list.sn}" class="table_link">
							        		${list.name}
							        	</a>
							    	</td>
							    	<td>
							    		<c:choose>
							    			<c:when test="${list.category eq 1 }">기업/단체</c:when>
							    			<c:otherwise>개인</c:otherwise>
							    		</c:choose>
							    	</td>
							    	<td>
							    		<c:choose>
							    			<c:when test="${list.category eq 3 }">구직회원</c:when>
							    	...
							    	<td>
							    		<c:choose>
							    			<c:when test="${list.isApproved eq -1 }">거절</c:when>
							    			<c:when test="${list.isApproved eq 0 }">승인대기</c:when>
							    			<c:otherwise>승인</c:otherwise>
							    		</c:choose>
							    	</td>
							    	<td>
							    		<c:choose>
							    			<c:when test="${list.isApproved eq -1 }">
							    				<a href="javascript:void(0)" class="admin_btn_style_1 skyblue" onclick="memberList.updateApprove(${list.sn},1)"><span>승인</span></a>
							    			</c:when>
							    			<c:when test="${list.isApproved eq 0 }">
							    				<a href="javascript:void(0)" class="admin_btn_style_1 skyblue" onclick="memberList.updateApprove(${list.sn},1)"><span>승인</span></a>
									    		<a href="javascript:void(0)" class="admin_btn_style_1 grey" onclick="memberList.updateApprove(${list.sn},-1)"><span>거절</span></a>
							    			</c:when>
