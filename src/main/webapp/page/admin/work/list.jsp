						...
						<table class="col-12">
							...
							<tbody>
							    ...
							    <tr>
							    	...
							    	<td>
							    		<a href="/adm/work/view?sn=${list.sn}&${searchDto.qustr}" class="table_link">
							        		${list.title}
							        	</a>
							    	</td>
							    	<td>
							    		<c:choose>
							    			<c:when test="${list.memCategory ne 1}">
							    				<c:choose>
									        		<c:when test="${list.classType eq 0}">
									        			<c:choose>
											        		<c:when test="${list.status eq 0}">미선정</c:when>
											        		<c:when test="${list.status eq 1}">선정</c:when>
											        		<c:when test="${list.status eq 2}">완료</c:when>
											        		<c:when test="${list.status eq 3}">신고</c:when>
											        		<c:otherwise>
											        			지급취소
											        		</c:otherwise>
											        	</c:choose>		
							    	...
							    	<td>
							    		<c:choose>
							        		<c:when test="${list.memCategory eq 0}">개인</c:when>
							        		<c:otherwise>
							        			기업/단체
							        		</c:otherwise>
							        	</c:choose>
							    	...
							    	<td>
							    		<c:choose>
							        		<c:when test="${list.classType eq 0}">구인</c:when>
							        		<c:otherwise>
							        			구직
							        		</c:otherwise>
							        	</c:choose>
							    	...
							    	<td>
							    		<c:choose>
											<c:when test="${list.startDateStr eq '0000-00-00'}">
												협의
											</c:when>
											<c:otherwise>
												${list.startDateStr} ${list.startTime} ~ ${list.endDateStr} ${list.endTime}
											</c:otherwise>
										</c:choose>
							    	</td>
							    </tr>  
