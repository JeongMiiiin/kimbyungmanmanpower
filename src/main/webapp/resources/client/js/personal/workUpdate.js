...
let workUpdate = {
	...
	,updateForm : function(sn, pageCategory){
		if(confirm('변경하시겠습니까?')){
			...
			let params = JSON.stringify({sn : sn, title : title, region : region, category : category, contents : contents, startDate : workStartDate, endDate : workEndDate, payment : workPayment});
			...
			if(workClassType != 1){
				...
				params = JSON.stringify({sn : sn, title : title, sex : sex, region : region, category : category, contents : contents, startDate : workStartDate, endDate : workEndDate, payment : workPayment});	
			} 
			...
			$.ajax({
				...
				,success : function(result){
					if(result.result){
						alert('변경되었습니다.');
						window.location.href='/personal/myInfo/workView?sn=' + sn + '&pageCategory=' + pageCategory;
					} else {
		...
		if(confirm('삭제하시겠습니까?')){
			...
			$.ajax({
					...
					,success : function(result){
						if(result.result){
							alert('변경되었습니다.');
							window.location.href='/personal/myInfo/page?tabIdx=0';
						}
					}
					,error:function(request,status,error){
				    	alert("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error);
