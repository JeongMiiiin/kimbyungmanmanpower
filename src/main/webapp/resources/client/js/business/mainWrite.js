let mainWrite = {
	...
	,open2DepthCategoryWritePopup : function(depth){
		let _this = this;
		if(depth == 1){
			_this.change2DepthCategoryWrite(1, $('#categoryWrite0').val());
		} else {
	...
	,change2DepthCategoryWrite(depth, category0){
		let _this = this;
	...
	,addCategoryWrite : function(){
		let currentList = $('#categoryWriteSelectPopList').val().substring(0,$('#categoryWriteSelectPopList').val().indexOf(']'));
		let addTarget = $('#categoryWrite0').val();
		if(currentList == '['){
			currentList += addTarget;
		} else {
	...
	,complete2DepthCategoryWrite : function(){
		if(confirm('적용하시겠습니까?')){
			...
			if($('#categoryWriteSelectPopList').val() != '[]'){
				let currentList = $('#categoryWriteList').val().substring(1, $('#categoryWriteList').val().length - 1).split(",");
				currentList.forEach(function(value){
					$('#workWriteCategoryList').append('<li><div><span>' + value + '</span><a href="javascript:void(0)" class="popup_work_select_list_delete_btn" onclick="mainWrite.deleteRegionWrite(\'' + value + '\', $(this), 0)"></a></div></li>')
				});	
			}
	...
	,completeWorkWritePopup : function(){
		if(confirm('등록하시겠습니까?')){
			...
			let sex = $('[name="workSex"]:checked').val();
			...
			/*if(workStartDate == '' || typeof(workStartDate) == "undefined"){workStartDate = "0000-00-00 00:00";}*/
			let workEndDate = $('#dateWriteEndDate').val();
			if(workEndDate == '' || typeof(workEndDate) == "undefined"){alert('종료기간을 입력해주세요'); return false;}
			/*if(workEndDate == '' || typeof(workEndDate) == "undefined"){workEndDate = "0000-00-00 00:00";}*/
			let workMemCategory = $('#workMemCategory').val();
			let workClass = $('#workClass').val();
			if(workClass == '' || typeof(workClass) == "undefined"){alert('분류를 선택해주세요'); return false;}
			...
			let params = JSON.stringify({title : title, password : password, sex : sex, region : region, category : category, contents : contents, startDate : workStartDate, endDate : workEndDate, memCategory : workMemCategory, classType : workClass, payment : workPayment, numbers : workNumbers});
			$.ajax({
				url : "/work/insertData"
